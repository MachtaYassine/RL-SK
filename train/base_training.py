    ...
    while not done:
        ...
        if current_round > last_round:
            ...
            for i, ag in enumerate(agents):
                ...
                round_score_for_agent = (reward[i]/last_round+20)
...
def run_base_training(args, env, agents, logger, writer=None,
                      start_episode=0, resumed_rewards=None, resumed_losses=None,
                      checkpoint_fn=None, checkpoint_every=1000):
    round_score_loss_coef = 0.0001

    # Restore or initialize metrics
    if resumed_rewards is not None:
        all_rewards = resumed_rewards
    else:
        all_rewards = [[] for _ in range(args.num_players)]

    if resumed_losses is not None:
        all_losses, all_bid_losses, all_play_head_losses, all_policy_losses = resumed_losses
        all_total_rewards = []
    else:
        all_losses = [[] for _ in range(args.num_players)]
        all_bid_losses = []
        all_play_head_losses = []
        all_policy_losses = []
        all_total_rewards = []
    ...
    for episode in range(start_episode, args.num_episodes):
        ...
        logger.info(f"--- Episode {episode+1}/{args.num_episodes} complete ---", color="green")
        logger.info(f"Episode rewards: {ep_rewards}", color="green")

        # Periodic checkpoint
        if checkpoint_fn and (episode + 1) % checkpoint_every == 0:
            checkpoint_fn(episode + 1, all_rewards,
                         (all_losses, all_bid_losses, all_play_head_losses, all_policy_losses))

    # Final checkpoint
    if checkpoint_fn:
        checkpoint_fn(args.num_episodes, all_rewards,
                     (all_losses, all_bid_losses, all_play_head_losses, all_policy_losses))
