...
def run_training(args, env, agents, logger, writer, logs_dir=None, start_episode=0, resumed_rewards=None, resumed_losses=None):
    if args.training_regimen == "sub_episode":
        ...
        return run_base_training(args, env, agents, logger, writer,
                                  ...
                                  resumed_rewards=resumed_rewards,
                                  resumed_losses=resumed_losses,
                                  checkpoint_fn=lambda ep, rw, lo: save_checkpoint(agents, args, logs_dir, ep, rw, lo) if logs_dir else None,
...
def save_checkpoint(agents, args, logs_dir, episode, all_rewards, all_losses):
    """Save a full checkpoint: model weights, optimizer state, episode counter, and metrics."""
    checkpoint = {
        'episode': episode,
        'args': vars(args),
    ...
    for i, agent in enumerate(agents):
        if hasattr(agent, 'bid_net'):
            checkpoint[f'agent_{i}_bid_net'] = agent.bid_net.state_dict()
            checkpoint[f'agent_{i}_play_net'] = agent.play_net.state_dict()
            checkpoint[f'agent_{i}_trick_predictor'] = agent.trick_win_predictor.state_dict()
            checkpoint[f'agent_{i}_optimizer_bid'] = agent.optimizer_bid.state_dict()
            checkpoint[f'agent_{i}_optimizer_play'] = agent.optimizer_play.state_dict()
            if args.shared_networks:
                break  # Only one copy needed for shared networks
    path = os.path.join(logs_dir, 'checkpoint.pt')
    torch.save(checkpoint, path)
    logging.getLogger(__name__).info(f"Checkpoint saved at episode {episode} to {path}")

def load_checkpoint(agents, args, checkpoint_path):
    """Load checkpoint and restore agent state. Returns (start_episode, all_rewards, all_losses)."""
    checkpoint = torch.load(checkpoint_path, map_location='cpu')
    for i, agent in enumerate(agents):
        if hasattr(agent, 'bid_net'):
            key = f'agent_{0 if args.shared_networks else i}'
            agent.bid_net.load_state_dict(checkpoint[f'{key}_bid_net'])
            agent.play_net.load_state_dict(checkpoint[f'{key}_play_net'])
            agent.trick_win_predictor.load_state_dict(checkpoint[f'{key}_trick_predictor'])
            agent.optimizer_bid.load_state_dict(checkpoint[f'{key}_optimizer_bid'])
            agent.optimizer_play.load_state_dict(checkpoint[f'{key}_optimizer_play'])
            if args.shared_networks:
                break
    start_episode = checkpoint['episode']
    all_rewards = checkpoint.get('all_rewards', None)
    all_losses = checkpoint.get('all_losses', None)
    logging.getLogger(__name__).info(f"Resumed from checkpoint at episode {start_episode}")
    return start_episode, all_rewards, all_losses
...
def main():
    ...
    if args.enable_tensorboard:
        ...
        # Launch TensorBoard only when not running headless on SLURM
    ...
    start_episode = 0
    resumed_rewards = None
    resumed_losses = None
    if args.resume_from:
        ckpt_path = os.path.join(args.resume_from, 'checkpoint.pt')
        if os.path.exists(ckpt_path):
            start_episode, resumed_rewards, resumed_losses = load_checkpoint(agents, args, ckpt_path)
        else:
    ...
    # Skip interactive mode on headless/SLURM
    if os.environ.get('SLURM_JOB_ID') is None:
        run_interactive(args, agents, logger)
