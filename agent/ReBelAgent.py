...
class ReBelValueNN(nn.Module):
    ...
    def __init__(self, max_players=5, max_rounds=10, hidden_dim=128):
        ...
        self.input_dim = self.max_players * (2 * self.max_rounds + 1)  # (11*5) + (5*10) = 105
    ...
    def forward(self, pbs, set_of_hands):
        ...
        x = torch.cat((padded_pbs.flatten(), padded_hands.flatten()), dim=0)  # Shape: (input_dim,)
...
class ReBelPolicyNN(nn.Module):
    ...
    def __init__(self, max_players=5, max_rounds=10, hidden_dim=128):
        ...
        # Compute input and output dimensions
        self.input_dim = (2 * self.max_rounds + 1) * self.max_players + self.max_rounds  # (11*5) + 10 = 115
    ...
    def forward(self, pbs, hand):
        ...
        # Flatten and concatenate
        x = torch.cat((padded_pbs.flatten(), padded_hand.flatten()), dim=0)  # Shape: (input_dim,)
...
def get_sets_of_hands(n_players, n_rounds, K, deck):
    ...
    for _ in range(K):
        ...
        hands = np.array_split(deck[: n_players * n_rounds], n_players)
...
def get_bids(set_of_hand, bidding_network):
    ...
    bids = torch.stack([bidding_network(hand).unsqueeze(0) for hand in set_of_hand])
...
def get_initial_pbs(set_of_hand, bids, N):
    ...
    Args:
        ...
        N (int): Number of possible public states from a set of hands that will be sampled.
    ...
    for _ in range(N):
        ...
        for i in range(num_played_cards):
            ...
            available_cards = hands_copy[current_player][hands_copy[current_player] != -1]

            if len(available_cards) > 0:
                chosen_card = np.random.choice(available_cards.numpy())  # Pick a random card
                played_cards.append((current_player, chosen_card))

                # Remove the card from the player's hand
                hands_copy[current_player, hands_copy[current_player] == chosen_card] = -1

        # Fill the played cards into the public state matrix
        round_idx = 1
        for i, (player, card) in enumerate(played_cards):
            pbs[round_idx, player] = card
            if (i + 1) % n_players == 0:  # Move to the next round after all players played
                round_idx += 1
...
def is_terminal(pbs, simu_depth):
    ...
    tricks_played = torch.sum((pbs[1:] != -1).any(dim=1))  # Count rows with at least one played card
    total_tricks = pbs.shape[0] - 1  # Total number of tricks (excluding bids row)
    
    tricks_left = total_tricks - tricks_played

    return tricks_left < simu_depth
...
def initialize_tree(pbs, simu_depth, set_of_hands, policy_network, warm_start):
    ...
    while d < simu_depth:
        for _ in range(set_of_hands.shape[0]):  # Ensure all players play one trick before incrementing d
            new_nodes = []
            leaves = [node for node in G.nodes if G.out_degree(node) == 0]  # Get current leaves
            ...
            for leaf in leaves:
                ...
                trick_idx, current_player = torch.nonzero(mask, as_tuple=True)[0]  # Get first empty trick slot
                current_player = current_player.item()

                # Get the player's remaining hand
                played_cards = set(current_pbs[1:].flatten().tolist()) - {-1}  # Exclude -1 (unplayed)
                current_hand = [card for card in set_of_hands[current_player] if card not in played_cards]
                ...
                if warm_start:
                    hand_tensor = torch.tensor(current_hand, dtype=torch.float32).view(1, -1)
                    proba = policy_network(hand_tensor).softmax(dim=1).squeeze(0)
                else:
                ...
                for card, prob in zip(current_hand, proba.tolist()):
                    ...
                    new_pbs[trick_idx + 1, current_player] = card  # Update trick row for this player
...
def get_expected_value(G, value_network, set_of_hands):
    """
    ...
    """
    expected_value = torch.zeros(set_of_hands.shape[0])  # Initialize expected values for all players
    ...
    for leaf in leaves:
        ...
        if (pbs[1:] == -1).sum() == 0:  
            values = compute_final_scores(pbs)  # Compute exact scores
        else:
            values = value_network(torch.cat((pbs.flatten(), set_of_hands.flatten())).unsqueeze(0))  # Estimate scores
...
def compute_final_scores(pbs, bids):
    """
    ...
    for t in range(1, n_rounds + 1):  # Iterate over tricks (excluding bid row)
        ...
        for i in range(n_players):
            ...
            if card.max() == -1:  # If -1, the player hasn't played (not possible in a complete game)
                continue
            ...
            if suit == atout_suit:  # Atout beats all non-atout cards
                if winner is None or (trick[winner][atout_suit] < value):
                    highest_card = value
                    winner = i
            elif suit == lead_suit and winner is None or (suit == lead_suit and highest_card < value):
                highest_card = value
                winner = i
...
def sample_leaf(G):
    ...
    for leaf in leaves:
        proba = 1.0
        path = list(nx.shortest_path(G, source=list(G.nodes)[0], target=leaf))  # Get path from root to leaf

        for i in range(len(path) - 1):
            proba *= G[path[i]][path[i + 1]]["weight"]  # Multiply probabilities along the path

        if proba > max_proba:
            max_proba = proba
            best_leaf = leaf

    return torch.tensor(best_leaf)  # Return the public state with highest probability


def update_policy(G, value_network, set_of_hands):
    """
    ...
    for edge in G.edges:
        parent_pbs, child_pbs = edge  # The edge represents a move from parent_pbs to child_pbs

        if parent_pbs not in subgame_dict:
            subgame_dict[parent_pbs] = get_expected_value(G.subgraph(nx.descendants(G, parent_pbs)), value_network, set_of_hands)
        
        if child_pbs not in subgame_dict:
            subgame_dict[child_pbs] = get_expected_value(G.subgraph(nx.descendants(G, child_pbs)), value_network, set_of_hands)

        player = torch.where(parent_pbs == -1)[1][0].item()  # Identify the player who played the move

        # Compute regret: max(0, value of taking action - value of parent state)
        regret = max(0, subgame_dict[child_pbs][player] - subgame_dict[parent_pbs][player])
...
def get_average_tricks_won(value_network_training_dict, n_players, n_rounds):
    """
    ...
    total_tricks = torch.zeros(n_players)  # Initialize trick count tensor
    count = len(value_network_training_dict)  # Number of hands evaluated

    for value in value_network_training_dict.values():  # value is a tensor of shape (n_players,)
        ...
        tricks_won[(value < 0) & (value != -10 * n_rounds)] = torch.abs(value[(value < 0) & (value != -10 * n_rounds)]) / 10  # If negative but not -10 * n_rounds, divide by -10
        tricks_won[value == -10 * n_rounds] = 1  # If exactly -10 * n_rounds, player bid 0 but won at least 1 trick

        total_tricks += tricks_won  # Accumulate tricks won

    return total_tricks / count if count > 0 else total_tricks  # Avoid division by zero
